...
$(document).ready(function() {
    cargarDatos(1, "cuerpoVentas");
    cargarDatos(2, "clientes");
    cargarDatos(3, "cmbCaja");
    cargarDatos(4, "cmbCajero");
    cargarDatos(20, "cmbCantCuotaVentas");
    $("#txtFechaVencCuota").datepicker();
    $("#btnModificar").hide();
    $("#btnBorrar").hide();
    ...
    $("#cmbTipoVenta").on('change', function() {
        //alert($(this).val());
        if ($(this).val() == "CON") {
            $("#divColCuotas").hide();
            $("#divColFecha").hide();
    ...
    $("#txtCodBarraBusqueda").on("keyup", function() {
        //setTimeout(buscarArticuloxCodigoBarra(), 20000);
        setTimeout(function() {
            buscarArticuloxCodigoBarra();
            var campos, valores = [];
            var id_stock;
            campos = $.trim($("#txtArticulo2").val()).split("|");
            valores = $.trim(campos[0]).split(":");
            id_stock = $.trim(valores[1]);
            buscarCostoVentasxId(id_stock);
            buscarExistenciaxId($("#cmbCostoVenta").val());
    ...
    $("#txtArticulo").change(function() {
        var campos, valores = [];
        var id_articulo;
        campos = $.trim($(this).val()).split("|");
        valores = $.trim(campos[0]).split(":");
        id_articulo = $.trim(valores[1]);

        buscarCostoVentasxId(id_articulo);
        buscarExistenciaxId($("#cmbCostoVenta").val());
        
        $("#cantidadArticulo").focus();
    });
    
    $("#cmbCostoVenta").on("change", function() {
        buscarExistenciaxId($(this).val());
    });

    /*$("#txtArticulo2").change(function() {
     var campos, valores = [];
     var id_stock;
     campos = $.trim($(this).val()).split("|");
     valores = $.trim(valores[0]).split(":");
     id_stock = $.trim(valores[1]);
...
function buscarCostoVentasxId(id_articulo) {
...
function buscarExistenciaxId(id_stock) {
    /*var camposArticuloSelect, valores3 = [];
     var id_stock;
     if ($("#chkBusquedaCodBarra").is(":checked")) {
     camposArticuloSelect = $.trim($("#txtArticulo2").val()).split("|");
     ...
     valores3 = $.trim(camposArticuloSelect[0]).split(":");
     id_stock = $.trim(valores3[1]);*/
   var valorCombo = $.trim(id_stock).split("-");
   var idStock = valorCombo[0];
   var costoArt = valorCombo[1];
    ...
    var resultado = $.ajax({
        ...
        dataType: 'html',
        success: function(datos) {
            $("#cmbExisArticulo").html($.trim(datos));
        },
        error: function() {
            alert("ERROR INTERNO : Comuníquese con soporte técnico.-");
        }
...
function accionProceso(opcion, id, valor) {
    ...
    datosJson = {
        ...
        "fecha_venc_cuota": $("#txtFechaVencCuota").val(),
        "cant_cuota": $("#cmbCantCuotaVentas").val(),
        "id": idv
    };
    if (validarCampos() && opcion != 3) {
...
function validarCampos() {
    if ($("#cmbTipoVenta").val() == "CRE") {
        if ($("#txtFechaVencCuota").val() == "")
            return true;
    }
    return false;
}
...
function accionProcesoDetalleVenta(opcion, id) {
   ...
   var valorCombo = [];
   var idStock, costoArt;
   valorCombo = $.trim($("#cmbCostoVenta").val()).split("-");
   idStock = valorCombo[0];
   costoArt = valorCombo[1];
    datosJson = {
        "opcion": opcion,
        "id_venta": $("#txtIdVenta").val(),
        "cantidad_venta": $("#cantidadArticulo").val(),
        "precio_unitario": costoArt,
...
function validarCantidadNosupereExistencia() {
    ...
    existencia = $("#cmbExisArticulo").val();
