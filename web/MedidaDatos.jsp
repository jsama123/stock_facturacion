...
<%
    ...
    switch(Integer.parseInt(opcion)){
        case 1:
            rs = dataSource.obtenerDato("SELECT id_medida, medida_descripcion FROM stock_facturacion.medidas a "
            ...
            while(rs.next()){
                htmlDatos.append("<tr>"
                        ...
                        + "<td>"+rs.getString("medida_descripcion") +"</td>"
                        + "<td><img src='imagenes/edit.png' style='cursor:pointer' onclick=\"abrirFormulario('modificar', "+rs.getString("id_medida")+")\" "
                        + "data-toggle='modal' data-target='#modalForm'></td>"
                        + "<td><img src='imagenes/delete.png' style='cursor:pointer' onclick=\"abrirFormulario('eliminar', "+rs.getString("id_medida")+")\" "
                        + "data-toggle='modal' data-target='#modalForm'></td>"
                        + "</tr>");
            }
            break;
        case 2:
            rs = dataSource.obtenerDato("SELECT medida_descripcion||'-'||id_medida as datos FROM stock_facturacion.medidas WHERE id_medida = "+id);
            if(rs.next()){
