    ...
    switch (Integer.parseInt(opcion)) {
        case 1:
            rs = dataSource.obtenerDato("SELECT  "
                    ...
                    + "a.existencia::int, "
                    + "a.cantidad, "
                    + "CASE WHEN operacion = 'S' THEN 'SUMA' ELSE 'RESTA' END as operacion,"
                    + "CASE WHEN estado = 0 THEN 'CREADO' WHEN estado = 1 THEN 'PROCESADO' ELSE 'ANULADO' END as estado,"
                    + "to_char(a.fec_alta::date, 'DD/MM/YYYY')as fecha "
                    ...
                    + "ORDER BY id_ajuste_stock desc;");
            while (rs.next()) {
                htmlDatos.append("<tr>"
                        + "<td>" + rs.getString("deposito_sucursal_descripcion") + "</td>"
                        + "<td>" + rs.getString("articulo_descripcion") + "</td>"
                        + "<td>" + rs.getString("existencia") + "</td>"
                        + "<td>" + rs.getString("cantidad") + "</td>"
                        + "<td>" + rs.getString("operacion") + "</td>"
                        + "<td>" + rs.getString("fecha") + "</td>"
                        + "<td>" + rs.getString("estado") + "</td>");
                if (rs.getString("estado").equals("CREADO")) {
                    htmlDatos.append("<td><img src='imagenes/edit.png' style='cursor:pointer' onclick=\"abrirFormulario('modificar', " + rs.getString("id_ajuste_stock") + ")\" "
                            + "data-toggle='modal' data-target='#modalForm'></td>"
                            + "<td><img src='imagenes/delete.png' style='cursor:pointer' onclick=\"abrirFormulario('eliminar', " + rs.getString("id_ajuste_stock") + ")\" "
                            + "data-toggle='modal' data-target='#modalForm'></td>"
                            + "<td><img src='imagenes/procesar.png' style='cursor:pointer; width:20px' onclick=\"aprobar(" + rs.getString("id_ajuste_stock") + ")\"></td>"
                            + "</tr>");
                } else {
        ...
        case 3:
            rs = dataSource.obtenerDato("SELECT "
                    + "'Cod:'||id_stock||' |Art :'||articulo_descripcion||' |Existencia :'||existencia::int||' |Costo :'||costo_compra::int as articulo "
